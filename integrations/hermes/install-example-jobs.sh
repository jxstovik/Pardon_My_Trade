...
post_week_prompt='Run the Pardon My Trade post-week model update from the configured absolute workdir using the PMT read-only MCP server and the attached pre-check output. If it says wakeAgent=false, report nothing and stop; that is the intentional zero-token path when completed observations are not ready. If it says precheck=blocked, do not call PMT tools; report the exact reason and stop. If it says precheck=pass, call `mcp_pmt_read_pmt_get_inseason_status` and `mcp_pmt_read_pmt_get_current_scoring_period`, determine the latest completed week represented by PMT_HISTORICAL_DATA_PATH, and call `mcp_pmt_read_pmt_update_post_week_outcomes` with that season/week and the current tool timestamp as the causalCutoff. The MCP server reads the configured completed-observation file; do not pass future rows. Then call `mcp_pmt_read_pmt_get_model_status` and `mcp_pmt_read_pmt_list_pending_actions`. Report the cutoff, manifest, promotion decision, rollback reference, limitations, and failures without emitting secrets. Never approve/reject actions or start `pmt daemon` or `pmt serve --scheduler`.'
